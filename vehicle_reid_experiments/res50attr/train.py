...
def parse_args():
    ...
    parser.add_argument('--id_num_classes',
                        type=int,
                        default=Config.id_num_classes,
                        help='model classification num')
    parser.add_argument('--color_num_classes',
                        type=int,
                        default=Config.color_num_classes,
                        help='model classification num')
    parser.add_argument('--type_num_classes',
                        type=int,
                        default=Config.type_num_classes,
                        help='model classification num')
...
def train(train_loader, model, criterion, optimizer, scheduler, epoch, logger,
    ...
    for iter_index, sample in enumerate(train_loader):
        inputs, id_targets, color_targets, type_targets = sample["image"].cuda(
        ).float(), sample["vehicle_id"].cuda(), sample["color"].cuda(
        ), sample["type"].cuda()

        feat_combine, feat_id_pred, feat_color_pred, feat_type_pred = model(
            inputs)
        color_loss = criterion["color_loss"](feat_color_pred, color_targets)
        type_loss = criterion["type_loss"](feat_type_pred, type_targets)
        id_loss = criterion["id_loss"](feat_id_pred, id_targets)

        triplet_loss, _, _ = criterion["triplet_loss"](feat_combine,
                                                       id_targets)
        center_loss = 0.0005 * (criterion["center_loss"](feat_combine,
                                                         id_targets))
        loss = color_loss + type_loss + id_loss + triplet_loss + center_loss
        ...
        acc1, _ = accuracy(feat_id_pred, id_targets, topk=(1, 5))
        top1.update(acc1.item(), inputs.size(0))
        losses.update(loss.item(), inputs.size(0))

        if iter_index % args.print_interval == 0:
            logger.info(
                f"train: epoch {epoch:0>3d}, iter [{iter_index:0>4d}, {iters:0>4d}], lr: {scheduler.get_lr()[0]:.6f}, top1 acc: {acc1.item():.2f}%,color_loss:{color_loss.item():.2f},type_loss:{type_loss.item():.2f}, id_loss:{id_loss.item():.2f}, triplet_loss:{triplet_loss.item():.2f}, center_loss:{center_loss.item():.2f}, loss_total: {loss.item():.2f}"
            )
...
def main(logger, args):
    ...
    model = resnet50attrbackbone(args.pretrained, args.id_num_classes,
                                 args.color_num_classes, args.type_num_classes)
    ...
    criterion_list = {
        "color_loss": nn.CrossEntropyLoss().cuda(),
        "type_loss": nn.CrossEntropyLoss().cuda(),
        "id_loss": nn.CrossEntropyLoss().cuda(),
        "triplet_loss": TripletLoss(margin=0.3),
        "center_loss": CenterLoss(num_classes=args.id_num_classes).cuda()
    }
