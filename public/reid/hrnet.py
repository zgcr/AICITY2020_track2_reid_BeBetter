...
def _hrnet(arch, pretrained, progress, **kwargs):
    w = hrnet_w_config[arch]
    model = HighResolutionNet(w, **kwargs)
...
def HRNet_W18(pretrained=False, progress=True, **kwargs):
    return _hrnet("HRNet_W18", pretrained, progress, **kwargs)
...
class Hrnetw18Backbone(nn.Module):
    ...
    def forward(self, x):
        ...
        x_list = []
        for i in range(self.model.stage2_num_branches):
        ...
        x_list = []
        for i in range(self.model.stage3_num_branches):
        ...
        x_list = []
        for i in range(self.model.stage4_num_branches):
        ...
        # Classification Head
        y = self.model.incre_modules[0](y_list[0])
        for i in range(len(self.model.downsamp_modules)):
            y = self.model.incre_modules[i + 1](
                y_list[i + 1]) + self.model.downsamp_modules[i](y)

        y = self.model.final_layer(y)
...
def hrnetw18backbone(pretrained, num_classes):
    model = Hrnetw18Backbone(num_classes)
